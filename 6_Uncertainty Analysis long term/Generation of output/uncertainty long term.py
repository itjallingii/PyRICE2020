...
save_folder = pydice_folder + '//output//' 
...
if __name__ == "__main__":
    for principle in principles_list:
        ...
        with MultiprocessingEvaluator(RICE) as evaluator:
            ...
            file_name =  "results_uncertainty_analsysis_long_term_policies_second_half" + principles_list[principle_index] +"_runs_" + str(nfe) + ".tar.gz"
            save_results(results, file_name)

            print("uncertainty_analysis_long_term_first_half" + principles_list[principle_index] + " cycle completed")        
        
        with MultiprocessingEvaluator(RICE) as evaluator:
            results = evaluator.perform_experiments(scenarios=nfe, policies=policy_list[len(policy_list)//2:])
          
            file_name =  save_folder + "results_uncertainty_analsysis_long_term_policies" + principles_list[principle_index] +"_runs_" + str(nfe) + ".tar.gz"
            save_results(results, file_name)

            print("uncertainty_analysis_long_term " + principles_list[principle_index] + " cycle completed")
