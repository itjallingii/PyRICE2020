...
RICE.levers = [RealParameter('sr', 0.1, 0.5),
               ...
               #controls for prioritarian case
               RealParameter('growth_factor_prio', 1,1.04),
               IntegerParameter('prioritarian_discounting',0,1)]

RICE.outcomes =[ScalarOutcome('Lowest income per capita 2055', ScalarOutcome.MAXIMIZE),
...
constraints = [Constraint("Lowest income per capita 2305", outcome_names="Lowest income per capita 2305",
...
if __name__ == "__main__":
    for i in range(len(epsilon_list)):    
        ...
        with MultiprocessingEvaluator(RICE) as evaluator:
            ...
            results.to_csv("results_prioritarian_policy_gen_"+ str(epsilon_list[i]) +".csv")
            convergence.to_csv("con_prioritarian_policy_gen_" + str(epsilon_list[i]) +".csv")
            
        #results.to_csv("wcs.csv")
        #convergence.to_csv("wcs_con.csv")
        end = time.time()
        print('prioritarian policy optimization 1 generation time is ' + str(round((end - start)/60)) + ' minutes', flush=True)
...
#            
#            results.to_csv("results_prioritarian_policy_gen_worst_case_ref1"+ str(epsilon_list[i]) +".csv")
#            convergence.to_csv("con_prioritarian_policy_gen_worst_case_ref1" + str(epsilon_list[i]) +".csv")
#
#        end = time.time()
#        print('prioritarian policy optimization 2 generation time is '+ str(round((end - start)/60)) + ' minutes', flush=True)
