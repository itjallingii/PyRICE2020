...
class PyRICE(object):
    ...
    def __call__(self,
        ...
        **kwargs):
        ...
        POP_ssp = pd.read_excel("pop_ssp.xlsx",header = None)              
        GDP_ssp = pd.read_excel("Y_Gross_ssp.xlsx",header = None)
        regions_list = ["US", "OECD-Europe","Japan","Russia","Non-Russia Eurasia","China","India","Middle East","Africa","Latin America","OHI","Other non-OECD Asia"]
        ...
        self.Sigma_gr_RICE = np.zeros((12, self.steps))
        ...
        self.global_damages[0] = self.damages[:,0].sum(axis = 0)
        self.global_ouput[0] = self.Y[:,0].sum(axis = 0)
        ...
        for t in range(1,31): 
            ...
            if scenario_pop_gdp !=0:
                ...
                if t == 1:
                    for region in range(0,12):
                        self.region_pop[region,:] = POP_ssp.iloc[:,(scenario_pop_gdp-1) + (region * 5)]
                      
                        self.Y_gross[region,:] = GDP_ssp.iloc[:,(scenario_pop_gdp-1) + (region * 5)] / 1000
            ...
            if scenario_sigma == 1:   #low SSP AEEI
                #calculate the sigma growth and the emission rate development
                if t == 1:
                    self.Sigma_gr_RICE[:,t] = (self.sigma_growth_data[:,4] + (self.sigma_growth_data[:,2] - self.sigma_growth_data[:,4] )) 
                ...
                if t > 1 :
                    self.Sigma_gr_RICE[:,t] =  (self.sigma_growth_data[:,4] + (self.Sigma_gr_RICE[:,t-1] - self.sigma_growth_data[:,4]  ) * (1-self.sigma_growth_data[:,3] )) 
            ...
            self.E_worldwilde_per_year = self.E.sum(axis=0)  
            ...
            if t < 30:
                ...
                if(self.mat[t+1] < self.mat_lo):
                    self.mat[t+1] = self.mat_lo
            ...
            if self.year > self.backstopcompetitiveyear:
            ...
            #calculate disaggregated per capita consumption based on income shares BEFORE damages
            self.CPC_pre_damage[self.year] = ((self.pre_damage_total__region_consumption[:,t] * self.RICE_income_shares.transpose() )  / (self.region_pop[:,t] * (1 / 5))) * 1000

            #calculate disaggregated per capita consumption based on income shares AFTER damages
            self.CPC_post_damage[self.year] = self.CPC_pre_damage[self.year]  - (((self.damages[:,t] *  self.damage_share.transpose() ) / (self.region_pop[:,t] * (1 / 5))) * 1000)
            
            #check for lower bound on C
            self.CPC_pre_damage[self.year] = np.where(self.CPC_pre_damage[self.year]  > CPC_lo, self.CPC_pre_damage[self.year], CPC_lo)
            self.CPC_post_damage[self.year] = np.where(self.CPC_post_damage[self.year]  > CPC_lo, self.CPC_post_damage[self.year], CPC_lo)
            
            #calculate damage per quintile equiv
            self.climate_impact_relative_to_capita[self.year] = ((self.damages[:,t] *  self.damage_share.transpose() * 10**12)/(0.2* self.region_pop[:,t]*10**6)) /(self.CPC_pre_damage[self.year] * 1000)
            
            
            self.climate_impact_relative_to_capita[self.year] = np.where(self.climate_impact_relative_to_capita[self.year] > 1,1,self.climate_impact_relative_to_capita[self.year])
            ...
            if self.welfare_function == "utilitarian":
                ...
                self.average_world_CPC[0] = (self.CPC[:,0].sum(axis=0) / 12)
                
                self.average_world_CPC[t] = (self.CPC[:,t].sum(axis=0) / 12)
                ...
                if t == 30:  
                    ...
                    self.intertemporal_utility_gini = diffsum / ((len(input_gini_inter_cpc)**2)* np.mean(input_gini_inter_cpc))
                    
                #intertemporal climate impact GINI
                self.average_regional_impact[t] = (self.damages[:,t].sum(axis=0) / 12)
                ...
                if t == 30:
                    ...
                    self.intertemporal_impact_gini = diffsum_inter / ((len(input_gini_inter)**2)* np.mean(input_gini_inter))
                ...
                #calculate gini as measure of current inequality in climate impact (per dollar consumption)  (intragenerational
                self.climate_impact_per_dollar_consumption[:,t] = np.where(self.damages[:,t]  < 0.001, self.CPC[:,t] , self.damages[:,t] / self.CPC[:,t])
                ...
                self.worst_off_income_class[t] = self.CPC_post_damage[self.year][0].min()
                self.worst_off_climate_impact[t] = self.climate_impact_relative_to_capita[self.year][0].max()
                ...
                #calculate utility equivalent for every income quintile and scale with welfare weights for comparison
                self.quintile_inst_util[self.year] = ((1 / (1 - self.elasmu)) * (self.CPC_post_damage[self.year])**(1 - self.elasmu) + 1)
                self.quintile_inst_util_ww[self.year] = self.quintile_inst_util[self.year] * self.Alpha_data[:,t]       
                
                utility_per_income_share = self.quintile_inst_util_ww[self.year]
            ...
            if self.welfare_function == "prioritarian":
                ...
                #cummulativie period utilty without WW
                self.cum_per_util[:,t] = self.cum_per_util[:,t-1] + self.per_util[:,t] 
                ...
                if self.prioritarian_discounting == 1:
                    #utility worst-off
                    self.inst_util_worst_off[:,t] = ((1 / (1 - self.elasmu)) * (self.CPC_post_damage[self.year][0])**(1 - self.elasmu) + 1)     
                    
                    self.inst_util_worst_off_condition[:,t] = ((1 / (1 - self.elasmu)) * (self.CPC_post_damage[self.year-10][0] * self.growth_factor)**(1 - self.elasmu) + 1)     
                ...
                array_worst_off_income = self.CPC_post_damage[self.year][0]
                self.worst_off_income_class_index[t] = np.argmin(array_worst_off_income)

                #objective for the worst-off region in terms of climate impact
                self.worst_off_climate_impact[t] = self.climate_impact_relative_to_capita[self.year][0].max()
                
                array_worst_off_share = self.climate_impact_relative_to_capita[self.year][0]
        ...
        ####################################################################
        """         
        #egalitarian objectives troubles 
        ...
        objectives_list_timeseries = [self.global_damages,self.global_per_util_ww,
                              self.worst_off_income_class,self.worst_off_climate_impact,
                              self.max_utility_distance_treshold,self.population_under_treshold,
                              self.CPC_intra_gini, self.climate_impact_per_dollar_gini,
                              self.temp_atm, self.E_worldwilde_per_year,self.global_ouput
                              ]

        #Eind en Y causing trouble

        objectives_list = [self.intertemporal_utility_gini,self.intertemporal_impact_gini,self.utility,self.regions_under_treshold]
        ...
        supplementary_list_timeseries = [self.CPC,self.region_pop]
        supplementary_list_quintile =[self.CPC_pre_damage,self.CPC_post_damage]
        ...
        for name in objectives_list_timeseries_name:
            for year in timepoints_to_save:
                ...
                self.data[name_year] = objectives_list_timeseries[index_var][int(timestep)]
            index_var = index_var + 1

        #save objectives with only one aggregate statistic over all timesteps
        index_var = 0 
        for name in objectives_list_name:
            self.data[name] = objectives_list[index_var]
            index_var = index_var + 1

        index_var = 0 
        #save additional data in timeseries format split within regions
        for name in supplementary_list_timeseries_name:
            for year in timepoints_to_save:
                ...
                self.data[name_year] = timestep_list

            index_var = index_var + 1

        index_var = 0 
        #save additional data in quintile 5 X 12 per year format
        for name in supplementary_list_quintile_name:
            for year in timepoints_to_save:
                ...
                self.data[name_year] = supplementary_list_quintile[index_var][year].tolist()
